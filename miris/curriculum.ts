  ...
  },
  {
    ...
    subs: [
      ...
      },
      {
        ...
        fill: "labelMesh",
        explain:
          "A plane with a meshBasicMaterial, nothing exotic. Billboard turns it to the camera every frame, the way every game nameplate works: without it, a plane is invisible edge-on and gone entirely from behind, because single-sided geometry culls its back face. transparent honours the rounded corners, toneMapped keeps the text out of the ACES curve, and label.width and height arrive already in scene units. Your HTML is now a surface in the world: occluded, screenshotted and streamed like everything else. Then try to select the text. You cannot, and that is the trade. HTML-in-Canvas normally keeps a drawn element selectable, focusable and readable by a screen reader, because the canvas showing it is the same canvas holding it, so the browser can map a hit region onto it. A Billboard gives it nothing to map: the plane turns to face you, hides behind your model and moves whenever you orbit. The pixels are the label now, and the DOM that made them is parked offscreen. Step 5.2 was the other end of the same trade, selectable and never once behind your model. Neither one is the right answer for every label.",
        renderPath: true,
        check: "labelMesh",
      },
      {
