  ...
  },
  {
    ...
    subs: [
      ...
      },
      {
        ...
        title: "Make them fit",
        body:
          "A streamed piece does not know how big it is at the moment it mounts. This adds the fit: wait for the SDK's streamloaded event, poll getBounds() at identity until the box stops moving, then scale into the niche volume and sit the piece bottom-centre on the shelf.",
        fill: "catalogFit",
        explain:
          "This is the one place a stream genuinely differs from a file, so it is worth being exact. A GLB has bounds the moment it parses; a stream's bounds converge, and the code therefore waits twice on two separate budgets. The first is network patience, 120s: before the asset is in, every poll reads an all-zero box, and counting those polls against the fit budget was really timing the connection, so six cold streams could burn the whole budget downloading and then be dropped as empty. The second is 40s for the box to settle once loaded, polled every 500ms, where settled means the characteristic size, cbrt(sx * sy * sz), moved less than 3% since the previous poll. Then scale by min(volume/size) times 0.9, so nothing touches the sides of its niche, and place it with its local minimum y on the shelf floor. Measure at identity and cache what you measured: after placement getBounds() reports the placed and scaled box rather than the clean local one, so anything that needs the piece's real size later, a placard for instance, must read your cached value rather than ask again. One renderer fix belongs to this step too, because streamed bounds are what causes it. Pin camera near to 0.1 and far to 60 every frame. The SDK derives them from bounds and had left near at 0.00035 against far 35.39, a 100000:1 ratio that collapses depth precision and z-fights anything sitting millimetres proud, which is what streaked the rug and the trim. Every frame, not once: a single assignment measured as being put straight back the next time bounds arrived.",
        check: "catalogFit",
      },
    ],
  },
